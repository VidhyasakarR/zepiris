...
## LSN Capture SDK (`lsn_capture_sdk/`, v1.0.0)
...
| Source package (`dist/lsn_capture_sdk-1.0.0.zip`) | 31 KB, 18 files |
...
**1. Copy the folder:** unzip `dist/lsn_capture_sdk-1.0.0.zip` into your project, then:
```yaml
dependencies:
  lsn_capture_sdk:
...
dependencies:
  lsn_capture_sdk:
    git:
      ...
      ref: dress-code-poc        # or a release tag, e.g. lsn_capture_sdk-v1.0.0
...
```bash
cd sdk && rm -f dist/lsn_capture_sdk-1.0.0.zip && zip -rq dist/lsn_capture_sdk-1.0.0.zip lsn_capture_sdk -x '*/build/*' '*/.dart_tool/*' '*.iml' '*/.idea/*' '*/.gradle/*' '*/local.properties'
```
